...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'northpine-leads' ),
					'singular_name' => __( 'Lead', 'northpine-leads' ),
					'edit_item'     => __( 'Lead details', 'northpine-leads' ),
					'search_items'  => __( 'Search leads', 'northpine-leads' ),
					'not_found'     => __( 'No leads yet. Submit the quote form to see one here.', 'northpine-leads' ),
					'all_items'     => __( 'All leads', 'northpine-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'northpine-leads' ),
			'npl_phone'    => __( 'Phone', 'northpine-leads' ),
			'npl_service'  => __( 'Needs', 'northpine-leads' ),
			'npl_timeline' => __( 'When', 'northpine-leads' ),
			'npl_zip'      => __( 'ZIP', 'northpine-leads' ),
			'npl_source'   => __( 'Source', 'northpine-leads' ),
			'npl_status'   => __( 'Status', 'northpine-leads' ),
			'date'         => __( 'Received', 'northpine-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'northpine-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'northpine-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Follow-up status', 'northpine-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'northpine-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'northpine-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'Email', 'northpine-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'northpine-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'northpine-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'northpine-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'northpine-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Consent given', 'northpine-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'northpine-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
