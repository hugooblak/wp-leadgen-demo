...
add_filter(
	...
	function ( $exporters ) {
		$exporters['northpine-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'northpine-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'northpine-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'northpine-leads',
						'group_label' => __( 'Quote requests', 'northpine-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['northpine-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'northpine-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
