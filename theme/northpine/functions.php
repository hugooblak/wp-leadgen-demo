<?php
/**
 * Northpine theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "Northpine Leads" plugin, so leads survive a theme change.
 *
 * @package Northpine
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'northpine', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'northpine' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'northpine' ) ) );

		register_block_pattern_category( 'northpine-sections', array( 'label' => __( 'Northpine sections', 'northpine' ) ) );
		register_block_pattern_category( 'northpine-pages', array( 'label' => __( 'Northpine page layouts', 'northpine' ) ) );
	}
);
